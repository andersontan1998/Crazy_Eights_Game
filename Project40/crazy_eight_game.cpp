...
{                           //and if it is empty then return true; if not, then return false
	if (handSize == 0)
		return true;
	else
		return false;
...
int main()
{
	...
	string playerEight; //when the player selects a crazy eight card, this will be used to determine what suite they want
	...
	bool win = false; //first declare that the win condition is false
	bool hasCard;
	...
	lastPile = firstCard; //the last card in the pile first starts as the first card dealed
	...
	while (d.size() > 0 && win == false)
	{
		...
		beginChoice: //for invalid responses go back here to start over
			...
			//for player that enter A,B,C instead of a,b,c
			...
			//the choice you choose cannot surpass the draw choice
			if (asciiPlayerChoice>p0.getHandSize())//makes sure that the player doesnt choose a choice that is not in range
			{
				...
				else if (!(asciiPlayerChoice == p0.getHandSize())) //if the player doesnt choose draw then it goes here
				{
					...
					beginSuit://to catch invalid responses go back here
						...
						cout << "You have declared suite " << lastPile.returnSuitString() << endl;
					...
					//if player chooses a card with the matching suit then do this
		...
		while (!hasCard && d.size() > 0 && win == false)
		{
			...
			if (p1.hasCardWithRank(lastPile.getRank(), lastPile))
			{
				...
				if (lastPile.getRank() == 8)
				{
					...
					cout << "  " << p1.getName() << " has declared suite " << p1.mostOccur() << endl;
		...
		while (!hasCard && d.size() > 0 && win == false)
		{
			...
			if (p2.hasCardWithRank(lastPile.getRank(), lastPile))
			{
				...
				if (lastPile.getRank() == 8)
				{
					...
					cout << "  " << p2.getName() << " has declared suite " << p2.mostOccur() << endl;
		...
		while (!hasCard && d.size() > 0 && win == false)
		{
			...
			if (p3.hasCardWithRank(lastPile.getRank(), lastPile))
			{
				...
				if (lastPile.getRank() == 8)
				{
					...
					cout << "  " << p3.getName() << " has declared suite " << p3.mostOccur() << endl;
